...
export class OrdersService {
  ...
  async addOrder(dto: CreateOrderDto): Promise<Order> {
    ...
    if (indPay) { total = total + +indPay }
    if (orderSale) { total = total - +orderSale }
    const order = await this.orderDB.create({ ...dto, orderTotal: total });
  ...
  async removeOrder(id: string, dto: RemoveTrashDto): Promise<Order> {
    ...
    let client = await this.supsService.validateClient(order.idClient);
    ...
    return this.orderDB.findByIdAndDelete(id);
  }
