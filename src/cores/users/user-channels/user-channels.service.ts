...
export class UserChannelsService {
  ...
  async addUserChannel(dto: AddUserChannelDto): Promise<UserChannel> {
    ...
    if (!mongoose.isValidObjectId(idUser)){ throw new HttpException({ message: `Ошибка - ID #${idUser} не корректен!` }, HttpStatus.BAD_REQUEST) }
    let user
    try { user = await this.userDB.findById(idUser); } catch (e) { console.log(e) }
    // Проверка наличия пользователя
    if(!user) {  throw new HttpException({ message: `Ошибка - пользователь с ID #${idUser} не найден!` }, HttpStatus.NOT_FOUND) }
