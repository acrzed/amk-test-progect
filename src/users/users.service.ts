...
import { ChannelName, ChannelNameDocument } from '../channel-names/entities/channel-name.entity';
...
export class UsersService {
  ...
  async removeUser(id: ObjectId, dto: RemoveUserDto): Promise<User> {
  ...
  async updDept(dto: DepartUpdateDto): Promise<User> {
    if ( !mongoose.isValidObjectId(dto.userID) ){ throw new HttpException({ message: `ID пользователя #${ dto.userID } не корректен!` }, HttpStatus.BAD_REQUEST)}
    try {
