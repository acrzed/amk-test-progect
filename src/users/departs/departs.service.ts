...
export class DepartsService {
  ...
  async create(createDepartDto: CreateDepartDto): Promise<Depart> {
  ...
  async findAll(): Promise<Depart[]> {
    try { return await this.departmentDB.find().exec(); } catch (e) { console.log(e) }
  }
  ...
  async update(id: ObjectId, updateDepartDto: UpdateDepartDto): Promise<Depart> {
    const { name, desc } = updateDepartDto
    if( !mongoose.isValidObjectId(id) ) {
      throw new HttpException({ message: `Ошибка - ID #${id} не корректен!` }, HttpStatus.BAD_REQUEST);
    }
    let updDept
    try { updDept = await this.departmentDB.findById(id) } catch (e) { console.log(e) }
    if ( !updDept ) {
